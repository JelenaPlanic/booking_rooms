...
const fileUpload = async (file, oldImage) => { // obrada i validacija fajlova (fajl je arg)
    ...
    if(error.length === 0) // cuvanje file na odredjenom mestu, na serveru
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            file.image.mv(UPLOAD_DIR + "/"  + fileName, (err) => { // mv = move(asihrona)
                ...
                {
                    if (oldImage && oldImage !== DEFAULT_USER_IMAGE && fs.existsSync(UPLOAD_DIR + oldImage)) {
                        fs.unlinkSync(UPLOAD_DIR + "/" + oldImage);
                    }
